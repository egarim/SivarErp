...
namespace Sivar.Erp.Documents
{
    ...
    public class TransactionGeneratorService
    {
        ...
        private readonly Dictionary<string, string> _accountMappings;        /// <summary>
        ...
        public TransactionGeneratorService(Dictionary<string, string>? accountMappings = null)
        {
            ...
            GenerateTransactionAsync(IDocument document)
        {
                ...
                (!string.IsNullOrEmpty(dto.DebitAccountCode) || !string.IsNullOrEmpty(dto.CreditAccountCode)))))
            {
                ...
                {
                    string accountId = totalDto.DebitAccountCode;

                    // Check if it's a mapping key first, otherwise use the code directly
                    if (_accountMappings.TryGetValue(totalDto.DebitAccountCode, out var mappedAccountId))
                    {
                ...
                {
                    string accountId = totalDto.CreditAccountCode;

                    // Check if it's a mapping key first, otherwise use the code directly
                    if (_accountMappings.TryGetValue(totalDto.CreditAccountCode, out var mappedAccountId))
                    {
                        accountId = mappedAccountId;
                    }
            ...
            return Task.FromResult((transaction, entries));
        }
