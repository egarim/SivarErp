...
{
    /// <summary>
    /// Infrastructure layer implementation of account import/export service (.NET 9)
    /// </summary>
    public class AccountImportExportService : IAccountImportExportService
    {
        ...
        /// <summary>
        /// Imports accounts from a CSV file using Core.Contracts interfaces
        /// </summary>
        ...
        public Task<(IEnumerable<CoreIAccount> ImportedAccounts, IEnumerable<string> Errors)> ImportFromCsvAsync(string csvContent, string userName)
        {
            List<CoreIAccount> importedAccounts = new List<CoreIAccount>();
            List<string> errors = new List<string>();

            if (string.IsNullOrEmpty(csvContent))
            {
                errors.Add("CSV content is empty");
                return Task.FromResult<(IEnumerable<CoreIAccount>, IEnumerable<string>)>((importedAccounts, errors));
            }
            ...
            try
            {
                ...
                for (int i = 1; i < lines.Length; i++)
                {
                    ...
                    // Validate account using legacy validator
                    ...
                    // Cast to CoreIAccount interface
                    importedAccounts.Add((CoreIAccount)accountDto);
                }
        ...
        public Task<string> ExportToCsvAsync(IEnumerable<CoreIAccount> accounts)
        {
        ...
        /// <summary>
        /// Creates an AccountDto from CSV fields
        /// </summary>
        ...
        /// <summary>
        /// Gets a CSV row for an account - handles potential missing properties safely
        /// </summary>
        /// <param name="account">Account to convert to CSV</param>
        /// <returns>CSV row as a string</returns>
        private string GetCsvRow(CoreIAccount account)
        {
            // Use reflection to safely access properties that might not exist in Core interface
            string balanceAndIncomeLineId = "";
            string parentOfficialCode = "";
            string accountTypeName = "Asset"; // Default
            
            // Check if account is actually an AccountDto (which has these properties)
            if (account is AccountDto accountDto)
            {
                balanceAndIncomeLineId = accountDto.BalanceAndIncomeLineId?.ToString() ?? "";
                parentOfficialCode = accountDto.ParentOfficialCode ?? "";
                accountTypeName = accountDto.AccountType.ToString();
            }
            else
            {
                // For other implementations, try to use reflection or Core properties
                try
                {
                    ...
                    if (accountTypeProperty != null)
                    {
                        ...
                        accountTypeName = accountTypeValue?.ToString() ?? "Asset";
                    }
                }
                catch
                {
            ...
            return $"\"{account.AccountName}\",\"{account.OfficialCode}\",{accountTypeName},\"{parentOfficialCode}\",{balanceAndIncomeLineId}";
        }
    }
}